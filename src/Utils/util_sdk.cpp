...
IMaterial *Util::CreateMaterial(std::string type, std::string texture, bool ignorez, bool nofog, bool model, bool nocull, bool halflambert)
{
	...
	materialData << "\"" + type + "\"\n"
					...
					texture + "\"\n"
							"\t\"$ignorez\" \"" +
					std::to_string(ignorez) + "\"\n"
										 "\t\"$nofog\" \"" +
					std::to_string(nofog) + "\"\n"
									    "\t\"$model\" \"" +
					std::to_string(model) + "\"\n"
									    "\t\"$nocull\" \"" +
					std::to_string(nocull) + "\"\n"
										"\t\"$halflambert\" \"" +
					std::to_string(halflambert) + "\"\n"
											"}\n"
			   << std::flush;
...
IMaterial *Util::CreateMaterial(bool ignorez, std::string envmap, std::string envmaptint, int envmapfresnel, std::string envmapfresnelminmaxexp)
{
	...
	KeyValues *keyValues = new KeyValues(materialName.c_str());
	InitKeyValues(keyValues, "VertexLitGeneric", 0, 0);
	LoadFromBuffer(keyValues, materialName.c_str(), materialData.str().c_str(), nullptr, nullptr, nullptr);
...
IMaterial *Util::CreateMaterial2(std::string type, std::string texture, bool ignorez, bool nofog, bool model, bool nocull, float alpha)
{
	static int matNum = 1;
	std::stringstream materialData;
	materialData << "\"" + type + "\"\n"
...
IMaterial *Util::CreateMaterial4(std::string type, std::string texture, bool ignorez, bool nofog, bool model, bool nocull, float alpha, Vector color)
{
	...
	materialData << "\"" + type + "\"\n"
							"{\n"
							"\t\"$envmaptint\" \"" +
					"[" + std::to_string(1) + " " + std::to_string(0) + " " + std::to_string(0) + "]" + "\"\n"
...
IMaterial *Util::CreateMaterial3(std::string type, std::string texture, bool ignorez, bool nofog, bool model, bool nocull, float alpha, Vector color, bool hasd)
{
	...
	materialData << "\"" + type + "\"\n"
					...
					"[0 1 2]" + "\"\n"
							  "\t\"$envmaptint\" \"" +
					"[" + std::to_string(0) + " " + std::to_string(0.5) + " " + std::to_string(0.55) + "]" + "\"\n"
