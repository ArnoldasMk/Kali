...
static C_BasePlayer* GetClosestPlayerAndSpot(CUserCmd* cmd, Vector* bestSpot, float* bestDamage, AimTargetType aimTargetType = AimTargetType::FOV)
{
	...
	if( lockedOn )
	{
		if( lockedOn->GetAlive() && !Entity::IsSpotVisibleThroughEnemies(lockedOn, lockedOn->GetBonePosition((int)Settings::Ragebot::bone)) )
		{
			lockedOn = nullptr;
			return nullptr;
		}
	...
	for (int i = 1; i < engine->GetMaxClients(); ++i)
	{
		...
		if(damage > damageAutoWall >= Settings::Ragebot::AutoWall::value && ( damage > 0.f && damageAutoWall > 0.f ))
		{
		...
		else if (damageAutoWall >= Settings::Ragebot::AutoWall::value && ( damage > 0.f && damageAutoWall > 0.f ))
		{
