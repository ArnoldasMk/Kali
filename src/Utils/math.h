...
namespace Math {
    inline bool Cmpf( float f1, float f2, float epsilon = 0.005f );
	...
	inline void VectorAngles(const Vector &forward, QAngle& angle);
	...
	inline void CalcAngle(const Vector &src, const Vector &dst, QAngle& angle);
	...
	inline float CalMaxDistance(const Vector &src, const Vector &dest);
	inline float CalMaxDistance(const QAngle &src, const Vector &dest);
	inline float SquareRoot(const float &x);
}

inline bool Math::Cmpf( float f1, float f2, float epsilon ) {
...
inline float Math::float_rand( float min, float max ) // thanks foo - https://stackoverflow.com/questions/13408990/how-to-generate-random-float-number-in-c :^)
{
...
inline void Math::VectorRotate(const float *in1, const matrix3x4_t& in2, float *out)
{
...
inline void Math::VectorRotate(const Vector &in1, const Vector &in2, Vector &out)
{
...
inline void Math::VectorRotate(const Vector& in1, const matrix3x4_t &in2, Vector &out)
{
...
inline void Math::NormalizeAngles(QAngle& angle)
{
	while (angle.x > 89.0f)
		angle.x -= 180.f;
...
inline void Math::NormalizeYaw( float& yaw ) {
	while ( yaw > 180.0f )
		yaw -= 360.0f;
	while ( yaw < -180.0f )
		yaw += 360.0f;
...
inline void Math::ClampAngles(QAngle& angle)
{
	if (angle.y > 180.0f)
		angle.y = 180.0f;
...
inline void Math::CorrectMovement(const QAngle &vOldAngles, CUserCmd* pCmd, float fOldForward, float fOldSidemove)
{
...
inline float Math::GetFov(const QAngle& viewAngle, const QAngle& aimAngle)
{
...
inline void Math::VectorAngles(const Vector& forward, QAngle &angles)
{
...
inline float Math::DotProduct( const float *v1, const float *v2 ) {
    return v1 [ 0 ] * v2 [ 0 ] + v1 [ 1 ] * v2 [ 1 ] + v1 [ 2 ] * v2 [ 2 ];
}

inline float Math::DotProduct(const Vector &v1, const float* v2)
{
...
inline void Math::CalcAngle(const Vector &src, const Vector &dst, QAngle& angle)
{
...
inline QAngle Math::CalcAngle(const Vector &src, const Vector &dst)
{
	QAngle angle;
	Vector delta = src - dst;
...
inline float Math::SquareRoot(const float &x)
{
	if ( x == 0 || x ==1)
		return x;
...
inline float Math::CalMaxDistance(const Vector &src, const Vector &dest)
{
	float	Ydistance = 0.f,
			ZDistance = 0.f;
...
inline float Math::CalMaxDistance(const QAngle &src, const Vector &dest)
{
...
inline void Math::AngleMatrix(const Vector angles, matrix3x4_t& matrix)
{
    float sr, sp, sy, cr, cp, cy;
...
inline void Math::VectorAngles(const Vector& forward, Vector& up, QAngle& angles)
{
