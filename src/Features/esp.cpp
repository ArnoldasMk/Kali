...
static bool GetBox(C_BaseEntity *entity, int &x, int &y, int &w, int &h)
{
	...
	if (debugOverlay->ScreenPosition(points[3], flb) || debugOverlay->ScreenPosition(points[5], brt) || debugOverlay->ScreenPosition(points[0], blb) || debugOverlay->ScreenPosition(points[4], frt) || debugOverlay->ScreenPosition(points[2], frb) || debugOverlay->ScreenPosition(points[1], brb) || debugOverlay->ScreenPosition(points[6], blt) || debugOverlay->ScreenPosition(points[7], flt))
		return false;
...
{
	float w = vMatrix[3][0] * origin.x + vMatrix[3][1] * origin.y + vMatrix[3][2] * origin.z + vMatrix[3][3];
	...
	screen->x = halfWidth +
			  (0.5f * ((vMatrix[0][0] * origin.x + vMatrix[0][1] * origin.y + vMatrix[0][2] * origin.z + vMatrix[0][3]) * inverseW) * width + 0.5f);

	screen->y = halfHeight -
			  (0.5f * ((vMatrix[1][0] * origin.x + vMatrix[1][1] * origin.y + vMatrix[1][2] * origin.z + vMatrix[1][3]) * inverseW) * height + 0.5f);
	return true;
}
...
static void DrawPlayer(C_BasePlayer *player)
{
	...
	if (!Entity::IsTeamMate(player, localplayer))
	{
		...
		if (Settings::ESP::FilterEnemy::Skeleton::enabled)
			DrawSkeleton(player, localplayer);

		if (Settings::ESP::FilterEnemy::HeadDot::enabled)
			DrawHeaddot(player);
	...
	else if (player != localplayer)
	{
		...
		if (Settings::ESP::FilterAlise::Skeleton::enabled)
			DrawSkeleton(player, localplayer);

		if (Settings::ESP::FilterAlise::HeadDot::enabled)
			DrawHeaddot(player);
	...
	else if (player == localplayer && Settings::ThirdPerson::toggled)
	{
		...
		if (Settings::ESP::FilterLocalPlayer::Skeleton::enabled)
			DrawSkeleton(player, localplayer);

		if (Settings::ESP::FilterLocalPlayer::HeadDot::enabled)
			DrawHeaddot(player);
